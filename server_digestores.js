// ============================================================================
// SERVER.JS — SISTEMA PREMIUM DE CONTROLE DE PROCESSO
// ============================================================================
...
// Criar pasta de relatórios se faltar
if (!fs.existsSync(REPORTS_DIR)) {
    fs.mkdirSync(REPORTS_DIR, { recursive: true });
    console.log("📂 Pasta reports criada em:", REPORTS_DIR);
}
...
// ============================================================================
// BROADCAST REAL-TIME (DIGESTORES, TOVAS, ENTRADAS)
// ============================================================================
...
app.get("/operador/historico", (req, res) => {
    db.all("SELECT * FROM digestor_discharges ORDER BY id DESC LIMIT 50", [], (err, rows) => {
...
app.get("/api/digestors", (req, res) => {
    db.all("SELECT * FROM digestors ORDER BY id", [], async (err, digestores) => {
        ...
        for (const d of digestores) {
            ...
            const cyc = await new Promise((resolve) =>
                db.get(
                    "SELECT * FROM cycles WHERE digestor_id=? AND status='in_progress' ORDER BY id DESC LIMIT 1",
                    [d.id],
                    (_, r) => resolve(r || null)
...
app.post("/api/cooking/start", (req, res) => {
    ...
    db.run(
        "UPDATE cooking_cycles SET start_cook_at=?, status='started' WHERE id=?",
        [now, cooking_id],
        function (err) {
            if (err) return res.status(500).json({ error: err.message });
            broadcastState();
