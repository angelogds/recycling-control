...
// Versão monolítica final: LOGIN, SESSÕES, DIGESTORES, PORTARIA, TOVAS, HISTÓRICO, PDF (opcional), SOCKET.IO, ADMIN CRUD
...
(async function main() {
  try {
    ...
    // continue anyway (attempt to open db will fail and server will show errors)
  }
  ...
  function broadcastState() {
    ...
    db.all("SELECT id, nome, capacidade_tn, status, last_cycle_id FROM digestors ORDER BY id", [], (err, digestores) => {
      ...
      Promise.all(tasks).then(resList => io.emit("digestors:update", resList)).catch(e => {
  ...
  // -------------------- API: trituraçao FINISH (auto start cooking) --------------------
    ...
    db.run("UPDATE cooking_cycles SET end_cook_at = ?, status = 'finished' WHERE id = ?", [now, cooking_id], function (err) {
      ...
      db.get("SELECT id, digestor_id FROM cycles WHERE cooking_id = ? AND status = 'in_progress' LIMIT 1", [cooking_id], (e, cyc) => {
        ...
        db.run("UPDATE cycles SET ended_at = ?, status = 'finished' WHERE id = ?", [now, cyc.id], (uErr) => {
          if (uErr) console.error("Err finish cycle:", uErr);
  ...
  app.get("/reports/cycle/:id", ensureAuth, (req, res) => {
    if (!gerarPDFCicloUtil) return res.status(500).send("PDF util não disponível. Coloque utils/pdf_ciclos.js no projeto.");
    const id = req.params.id;
  ...
  app.get("/admin/users/new", ensureAuth, ensureRole("admin"), (req, res) => {
    res.render("admin_users_new", { usuario: req.session.user });
  });
    ...
    try {
      ...
      db.run("INSERT INTO users (username, nome, role, password) VALUES (?, ?, ?, ?)", [username, nome || username, role || "operador", hash], (err) => {
        if (err) {
          ...
          return res.render("admin_users_new", { usuario: req.session.user, error: "Erro ao criar usuário" });
        }
  ...
  app.get("/admin/users/:id/edit", ensureAuth, ensureRole("admin"), (req, res) => {
    ...
    db.get("SELECT id, username, nome, role FROM users WHERE id = ?", [id], (err, row) => {
      ...
      res.render("admin_users_edit", { usuario: req.session.user, user: row });
    });
  });
  ...
  app.post("/admin/users/:id/update", ensureAuth, ensureRole("admin"), async (req, res) => {
    ...
    if (password && password.length > 0) {
      try {
        ...
        console.error("Err bcrypt update:", e);
  ...
  // ensure this runs *after* DB creation; if table 'users' doesn't exist skip gracefully
  db.get("SELECT name FROM sqlite_master WHERE type='table' AND name='users'", [], (err, tbl) => {
    ...
    if (!tbl) { console.warn("Tabela users não encontrada — verifique init_db.sql"); return; }
    ...
    db.get("SELECT COUNT(*) AS cnt FROM users", [], (err2, row) => {
      ...
      if (cnt === 0) {
        ...
        console.log("Users already seeded (count:", cnt, ")");
      }
    });
  });
